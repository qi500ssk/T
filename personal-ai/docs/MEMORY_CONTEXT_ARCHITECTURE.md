...
`PATCH` 中内容、类型或作用域发生变化时创建替代版本并写入 `supersedes_id`；
只有重要度或用户开关变化时原地更新。聊天中的显式“记住、查看记忆、纠正记忆、忘记”
通过 `memory_create/list/update/forget` 使用同一套服务，不再降级为工作区 Markdown 文件。
...
- 类型明确显示为偏好与身份、长期事实（语义记忆）、重要事件（情景记忆）；
- 按全局、项目、来源会话分组，并可按类型和作用域筛选；
- 显示真实 `usage_count`、`last_used_at`、来源会话、状态和完整替换历史；
- 手工新增可选择全局或具体项目，不再默认把所有内容写成全局记忆；
- “停用”只修改用户开关，“过期”和“被替换”由系统生命周期管理，永久删除单独确认。
...
### 15.3 2026-08-24 记忆治理收口

本轮按以下阶段完成设计与前端之间的最后一段断层：

1. **显式记忆入口**：为聊天增加确定性的新增、查看、纠正和忘记路由；记忆操作只允许
   `memory_*` 工具，不能调用文件、代码、Skill 或 MCP 代替数据库记忆。
2. **成功对账**：最终成功提示必须能在持久化 `ToolRun` 中找到对应的已完成工具；没有真实
   写入时明确提示失败，模型口头声称“记住了”不作为成功依据。
3. **生命周期治理**：统一纠正逻辑、替换链、完整历史、过期、停用、作用域移动和查询过滤；
   显式操作后跳过自动抽取，避免同一轮重复写入。
4. **前端对应**：记忆页完整展示种类、作用域、状态、来源、使用反馈和版本历史，支持项目级
   新增、纠正、恢复、过期与永久删除。
5. **路由边界**：解释“语义记忆是什么”仍是普通问答；“不要记住”优先进入忘记/拒绝写入
   路径；规则命中的记忆操作不触发额外意图模型。
6. **端到端验证**：覆盖聊天新增、纠正、忘记、不写替代文件、项目隔离、完整历史、过期过滤、
   真实使用计数和前端构建/静态检查。

最终验证结果：

- 后端：212 passed，1 skipped；
- 前端：`npm run build` 通过，`npm run lint` 0 error（2 条既有聊天图片性能提示）；
- 记忆评测：错误记忆率 0、重复率 0、Recall@3 1.000、作用域误召回率 0、冲突处理率 1.000、上下文实际使用率 1.000；
- 意图评测：Accuracy 1.000、规则短路率 1.000、规则用例模型往返 0；
- RAG 评测：Recall@1/3/5、MRR、章节命中和关键词命中均为 1.000；
- 真实页面：生产构建下完成数据加载、纠正表单、取消操作和替换历史展开验证，控制台 0 error。
