"""聊天域 Model Gateway：业务代码不直接绑定具体模型 API。
...
class MockProvider:
    ...
    async def stream(
        ...
        if "PLANNER_SYNTHESIZE_V1" in system_text:
            try:
                ...
                results = payload.get("completed_steps") or []
                detail = "；".join(str(item.get("output_summary") or "") for item in results)
                reply = f"（Mock 规划回复）{detail or payload.get('goal', '任务已处理')}"
            except (json.JSONDecodeError, AttributeError):
                reply = "（Mock 规划回复）任务已处理"
            async for chunk in self._stream_reply(reply):
                yield chunk
            return
        if "PLANNER_STEP_V1" in system_text:
            try:
    ...
    def _select_tool(text: str, available: set[str]) -> tuple[str, dict] | None:
        if "code_create_file" in available and "创建代码文件" in text:
            ...
            if match:
                return "code_create_file", {
                    "path": match.group(1).strip(),
                    "content": match.group(2).strip(),
                }
        if "code_edit" in available and "修改代码文件" in text:
            match = re.search(
                r"修改代码文件[：:\s]*([^，,\s]+)\s+将[：:]\s*(.*?)\s+改为[：:]\s*(.*)",
                text,
                flags=re.DOTALL,
            )
            if match:
        ...
        if "code_search" in available and "搜索代码" in text:
            match = re.search(r"搜索代码[：:\s]*(.+)", text, flags=re.DOTALL)
            return "code_search", {"query": (match.group(1) if match else text).strip()}
        if "code_read" in available and any(token in text for token in ("读取代码", "查看代码文件")):
            match = re.search(r"(?:读取代码|查看代码文件)[：:\s]*([^，。\s]+)", text)
            return "code_read", {"path": match.group(1) if match else "README.md"}
        if "code_list_files" in available and any(
            token in text for token in ("列出代码", "查看项目文件", "项目结构", "代码目录")
        ):
            return "code_list_files", {"path": "."}
        if "code_git_diff" in available and any(
            token in text.lower() for token in ("代码改动", "git diff", "未提交改动")
        ):
            return "code_git_diff", {}
        if "code_run_check" in available and "运行代码检查" in text:
            match = re.search(r"运行代码检查[：:\s]*([a-z-]+)", text.lower())
            return "code_run_check", {"check": match.group(1) if match else "python-compile"}
        ...
        if "calculate" in available and "计算" in text:
            ...
            expression = re.sub(r"^(?:出|一下)", "", expression).strip() or "0"
    ...
    async def complete(self, messages: list[dict], temperature: float = 0.0) -> str:
        ...
        if "PLANNER_CREATE_V1" in system:
            ...
            goal = str(payload.get("goal", "任务"))
            available = {item.get("name") for item in payload.get("available_tools", [])}
            selected = self._select_tool(goal, available)
            hint = selected[0] if selected else None
            first = {
                "title": "执行核心任务",
                "instruction": goal,
                "tool_hints": [hint] if hint else [],
            }
            return json.dumps(
                {
                    "goal": goal,
                    "steps": [
                        ...
                        {
                            "title": "整理结果",
                            "instruction": "根据已经得到的结果整理简洁结论，不再调用工具。",
                            "tool_hints": [],
                        },
                    ],
                },
        ...
        if "PLANNER_REPLAN_V1" in system:
            payload = json.loads(content)
            return json.dumps(
                {
                    "goal": str(payload.get("goal", "任务")),
                    "steps": [
                        {
                            "title": "安全替代说明",
                            "instruction": "说明当前操作受限，并给出不执行高风险动作的安全替代建议。",
                            "tool_hints": [],
                        }
                    ],
                },
