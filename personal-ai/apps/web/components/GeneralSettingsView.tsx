              ...
              <section className="rounded-3xl bg-zinc-100 p-5 ring-1 ring-zinc-200 sm:p-7" aria-label={selectedAgentId ? "编辑角色预设" : "新建角色预设"}>
                ...
                <div className="mt-6 grid gap-5 sm:grid-cols-2">
                  ...
                  <fieldset disabled={customPromptActive} className={`contents ${customPromptActive ? "opacity-45" : ""}`} aria-describedby="basic-settings-note">
                    ...
                    <label className="grid gap-2 text-sm font-medium">默认语言<SelectMenu value={agent.language} onChange={(value) => setAgent({ ...agent, language: value })} options={[{ value: "zh-CN", label: "简体中文" }, { value: "zh-TW", label: "繁体中文" }, { value: "en-US", label: "English" }, { value: "ja-JP", label: "日本語" }]} ariaLabel="默认语言" className={inputClass} /></label>
                    ...
                    <label className="grid gap-2 text-sm font-medium">回答长度<SelectMenu value={agent.verbosity} onChange={(value) => setAgent({ ...agent, verbosity: value })} options={["简洁", "适中", "详细", "根据问题自动调整"].map((label) => ({ value: label, label }))} ariaLabel="回答长度" className={inputClass} /></label>
                    <label className="grid gap-2 text-sm font-medium">幽默程度<SelectMenu value={agent.humor} onChange={(value) => setAgent({ ...agent, humor: value })} options={["关闭", "少量", "适度", "活泼"].map((label) => ({ value: label, label }))} ariaLabel="幽默程度" className={inputClass} /></label>
            ...
            <div className="mt-9 grid items-start gap-6 lg:grid-cols-[18rem_minmax(0,1fr)]">
              ...
              <section className="rounded-3xl bg-zinc-100 p-5 ring-1 ring-zinc-200 sm:p-7" aria-label={selectedModelId ? "编辑模型配置" : "新建模型配置"}>
                ...
                <div className="grid gap-5 sm:grid-cols-2">
                  ...
                  <label className="grid gap-2 text-sm font-medium sm:col-span-2">配置模板<SelectMenu value={preset === "mock" ? "custom" : preset} onChange={choosePreset} options={[{ value: "deepseek", label: "DeepSeek API" }, { value: "ollama", label: "Ollama 本地模型" }, { value: "custom", label: "其他 OpenAI 兼容服务" }]} ariaLabel="配置模板" className={inputClass} /></label>
                  ...
                  <label className="grid gap-2 text-sm font-medium">模型名称{preset === "deepseek" ? <SelectMenu value={model.model} onChange={(value) => setModel({ ...model, model: value })} options={[{ value: "deepseek-v4-flash", label: "DeepSeek V4 Flash" }, { value: "deepseek-v4-pro", label: "DeepSeek V4 Pro" }]} ariaLabel="模型名称" className={inputClass} /> : <input value={model.model} onChange={(e) => setModel({ ...model, model: e.target.value })} className={inputClass} placeholder="模型 ID" />}{preset === "deepseek" && <span className="text-xs font-normal leading-5 text-zinc-500">来自 DeepSeek 官方当前模型列表；旧的 deepseek-chat / reasoner 已停止使用。</span>}</label>
