...
import type { AgentSettings, Conversation, Project } from "@/lib/api";
  ...
  return <aside className="flex max-h-72 min-w-0 w-full shrink-0 flex-col overflow-hidden border-b border-zinc-200 bg-zinc-50 md:max-h-none md:w-72 md:border-b-0 md:border-r">
    ...
    <div className="hidden min-h-0 flex-1 flex-col md:flex">
      <div className="flex-1 overflow-y-auto px-2 pb-3">
        ...
        <div className="mb-1 flex min-h-8 items-center justify-between px-3"><p className="text-xs font-medium uppercase tracking-wider text-zinc-400">项目</p><button type="button" onClick={onCreateProject} className="grid size-7 place-items-center rounded-lg text-lg text-zinc-500 hover:bg-zinc-200" aria-label="新建项目" title="新建项目">＋</button></div>
        {projects.map((project) => <section key={project.id} className="mb-1"><button type="button" onClick={() => openProject(project.id)} className={`flex min-h-9 w-full items-center gap-2 rounded-lg px-3 text-left text-sm font-medium ${activeProjectId === project.id && view === "chat" ? "bg-zinc-200 text-zinc-950" : "text-zinc-700 hover:bg-zinc-100"}`} title={project.workspace_dir || project.name}><span className="text-zinc-400" aria-hidden="true">▱</span><span className="min-w-0 flex-1 truncate">{project.name}</span><span className="text-xs text-zinc-400">{grouped(project.id).length}</span></button>{taskRows(grouped(project.id))}</section>)}
        {projects.length === 0 && <p className="px-3 py-3 text-sm text-zinc-400">新建项目后，项目任务会显示在这里</p>}
      </div>
    </div>
